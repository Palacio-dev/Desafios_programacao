...
void read_reversed(bignum *n, string & line){
    n->signbit = PLUS;
...
void revert_bignum(bignum * n, bignum * invertido){
 ...
 int main(){
    bignum a, b, result, result_inverted;
    int n;
    string line1, line2;
    cin >> n;
    while (n--){
        initialize_bignum(&a); initialize_bignum(&b); initialize_bignum(&result); initialize_bignum(&result_inverted);
        cin >> line1 >> line2;
        read_reversed(&a, line1); read_reversed(&b, line2);
