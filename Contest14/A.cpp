...
int main(){
    ...
    while ((cin >> n) and n!= 0){
        ...
        if (points.size() == 1){
            cout << 1 << "\n";
            cout << points.begin()->x << " " << points.begin()->y << "\n";
            continue;
        }
        ConvexH = vector<point>(points.begin(), points.end());
        if (check(ConvexH[0], ConvexH[1], ConvexH)) {
             ConvexH.clear();
             ConvexH.push_back(*points.begin());
             ConvexH.push_back(*points.rbegin());         
             ConvexH.push_back(*points.begin());
        }
        else{
            ConvexH = CH(vector<point>(points.begin(), points.end()));
        }
        cout << ConvexH.size() -1  << "\n";
