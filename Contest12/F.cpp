...
struct line {
    ...
    // Construtor (só para facilitar)
    line() { a = b = c = 0.0; }
    
    // O operador< é necessário para o std::set
    bool operator<(const line& other) const {
        // Compara 'a', depois 'b', depois 'c' com tolerância EPS
...
int main(){
    ...
    // Este loop agora funciona, pois 'n' ainda é 4 (no exemplo)
    for (int i = 0; i < n; i++) {
        for (int j = i + 1; j < n; j++) {
            pointsToLine(points[i], points[j], l);
            lines.insert(l); // Insere apenas as linhas únicas
        }
    }

    vector<line> unique_lines(lines.begin(), lines.end());
