...
bool onSegment(point p, point q, point r) {
    ...
    bool in_x = (q.x <= max(p.x, r.x) + EPS) and (q.x >= min(p.x, r.x) - EPS);
    bool in_y = (q.y <= max(p.y, r.y) + EPS) and (q.y >= min(p.y, r.y) - EPS);
    return in_x and in_y;
}

bool segmentsIntersect(point p1, point p2, point p3, point p4) {
    double d1 = cross(toVec(p3, p4), toVec(p3, p1));
    double d2 = cross(toVec(p3, p4), toVec(p3, p2));
    double d3 = cross(toVec(p1, p2), toVec(p1, p3));
    double d4 = cross(toVec(p1, p2), toVec(p1, p4));

    if (d1 * d2 < -EPS && d3 * d4 < -EPS) {
        return true;
    }

    if (collinear(p1, p2, p3) && onSegment(p1, p3, p2)) return true;
    if (collinear(p1, p2, p4) && onSegment(p1, p4, p2)) return true;
    if (collinear(p3, p4, p1) && onSegment(p3, p1, p4)) return true;
    if (collinear(p3, p4, p2) && onSegment(p3, p2, p4)) return true;
    return false; 
}
...
int main() {
    ...
    while (n--) {
        cin >> m; 
        vector<pair<point, point>> segments(m);
        for (int i = 0; i < m; i++) {
